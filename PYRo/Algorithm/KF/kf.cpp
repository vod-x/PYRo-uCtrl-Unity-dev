...
status_t kf_t::init(float *A_data, float *B_data, float *H_data, float *Q_data, float *R_data)
{
	return init_impl(A_data, B_data, H_data, Q_data, R_data, nullptr, nullptr);
}

status_t kf_t::init(float *A_data, float *B_data, float *H_data, float *Q_data, float *R_data,
					float *x0_data)
{
	return init_impl(A_data, B_data, H_data, Q_data, R_data, x0_data, nullptr);
}

status_t kf_t::init(float *A_data, float *B_data, float *H_data, float *Q_data, float *R_data,
					std::nullptr_t, float *P0_data)
{
	return init_impl(A_data, B_data, H_data, Q_data, R_data, nullptr, P0_data);
}

status_t kf_t::init(float *A_data, float *B_data, float *H_data, float *Q_data, float *R_data,
					float *x0_data, float *P0_data)
{
	return init_impl(A_data, B_data, H_data, Q_data, R_data, x0_data, P0_data);
}

status_t kf_t::init_impl(float *A_data, float *B_data, float *H_data, float *Q_data, float *R_data,
					 const float *x0_data, const float *P0_data)
{
	/* 1. Validate input: all matrix data pointers must be non-null */
	if (A_data == nullptr || B_data == nullptr || H_data == nullptr || Q_data == nullptr || R_data == nullptr)
	{
	 ...
	 *    Q (x_size x x_size) : process noise covariance
...
status_t kf_t::update(float *measure_vec, float *control_vec, float *estimated_ret)
{
	 ...
	 *        P_{k}^- = A * P_{k-1} * A^T + Q
	 *    Temporary variables:
	 *      _tmp_xx_1 (x_size x x_size) = A * P_{k-1}
