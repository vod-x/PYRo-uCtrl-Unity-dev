...
static bool transmitRefereeUiPacket(const uint8_t* data, uint16_t length, void* context) {
    ...
    auto uart = pyro::uart_drv_t::get_instance(static_cast<pyro::uart_drv_t::which_uart>(REFEREE_UART));
...
void hudProducerTask(void *argument) {
    ...
    for (;;) {
        ...
        input.spinEnabled = (cmd.mode == cmd::SPIN);             
        input.stepClimbEnabled = (cmd.mode == cmd::STEP_CLIMB);  
...
status_t infantry2_chassis_init(void *argument)
{
    ...
    xTaskCreate(refereeUiRendererTask, "ui_renderer", 1024, nullptr, 1, nullptr);
    xTaskCreate(hudProducerTask,       "ui_producer", 1024, nullptr, 1, nullptr);
